...
void GameStateMain::handleEvent(const InputState& inputState) {
    ...
    if (inputState.keyboardState.isJustPressed(SDL_Scancode(fallKey))) {
        while (board->isPossibleMovement(currentPiece.x, currentPiece.y - 1, currentPiece.kind,
    ...
    if (inputState.keyboardState.isJustPressed(SDL_Scancode(rotateKey))) {
        if (board->isPossibleMovement(currentPiece.x, currentPiece.y - 1, currentPiece.kind,
...
void GameStateMain::drawBoard() {
    ...
    Color borderColor = Color(255, 0, 255, 255);
    gRenderer->drawRect(Vector2(left - BOARD_LINE_WIDTH, height), Vector2(BOARD_LINE_WIDTH, BLOCK_SIZE * BOARD_HEIGHT),
                        0.0f, borderColor);
    gRenderer->drawRect(Vector2(right, height), Vector2(BOARD_LINE_WIDTH, BLOCK_SIZE * BOARD_HEIGHT), 0.0f,
                        borderColor);
